...
class HierarchicalRNG:
    ...
    def get_rng(self, session_id: int, v_th_std: float, g_std: float, trial_id: int,
        ...
        v_th_int = int(v_th_std * 1000)  # 0.5 -> 500
        g_int = int(g_std * 1000)        # 0.3 -> 300

        # TRIAL-VARYING: Poisson processes and initial states change with trial_id
        if component in ['initial_state', 'static_poisson', 'dynamic_poisson_spikes']:
            seed_components = [self.base_seed, session_id, v_th_int, g_int, trial_id]
        # STRUCTURE: Network topology depends on session_id AND parameters (not trial)
        else:
            seed_components = [self.base_seed, session_id, v_th_int, g_int]

        # Add component-specific offset for additional randomization
        component_offset = abs(hash(component)) % 1000000
        seed_components.append(component_offset)

        # Always create completely fresh RNG instance - no caching at all
...
def get_rng(session_id: int, v_th_std: float, g_std: float, trial_id: int,
    ...
    return rng_manager.get_rng(session_id, v_th_std, g_std, trial_id, component)
