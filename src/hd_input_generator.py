# src/hd_input_generator.py - HD input generation for encoding experiments
"""
High-dimensional input generator for encoding capacity studies.
Uses rate RNN dynamics embedded in lower-dimensional subspaces.
"""
...
def run_rate_rnn(n_neurons: int, T: float, dt: float, g: float,
    ...
    # Remove transient (first 50ms = 500 steps at dt=0.1)
    transient_steps = int(50.0 / dt)
...
class HDInputGenerator:
    """High-dimensional input generator for encoding experiments."""
    ...
    def __init__(self, embed_dim: int = 10, dt: float = 0.1):
        ...
        self.Y_base = None  # Base embedding without noise
    ...
    def initialize_base_input(self, session_id: int, hd_dim: int,
        ...
        """
        Generate base HD input (fixed per session/hd_dim/embed_dim).
        ...
        Args:
            ...
            rate_rnn_params: Parameters for rate RNN
                ...
                - T: default 350ms (50ms transient + 300ms encoding)
                - g: default 1.2
        """
        if rate_rnn_params is None:
            rate_rnn_params = {'n_neurons': 1000, 'T': 350.0, 'g': 1.2}

        # Run rate RNN
        rates, _ = run_rate_rnn(
            n_neurons=rate_rnn_params.get('n_neurons', 1000),
            T=rate_rnn_params.get('T', 350.0),
            dt=self.dt,
