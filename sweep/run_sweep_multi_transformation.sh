#!/bin/bash
# sweep/run_sweep_multi_transformation.sh
# Temporal transformation task (4 patterns) - MODIFIED to use cached spikes
...
python3 "$SCRIPT_DIR/generate_jobs.py" \
    ...
    --output "$SCRIPT_DIR/jobs_multi_transformation.txt"
...
"$SCRIPT_DIR/run_sweep_engine.sh" \
    --jobs_file "$SCRIPT_DIR/jobs_multi_transformation.txt" \
    --task "$TASK_TYPE" \
    --num_parallel $NUM_PARALLEL \
    --logdir "$SCRIPT_DIR/logs_multi_transformation"
